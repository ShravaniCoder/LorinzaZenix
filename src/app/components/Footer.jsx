...
export function Footer() {
  ...
  /* ── Only trigger when user has scrolled to the absolute bottom of the page ── */
  useEffect(() => {
    const handleScroll = () => {
      if (isRevealed) return; // don't re-trigger while already open
      const scrollTop = window.scrollY || document.documentElement.scrollTop;
      const scrollHeight = document.documentElement.scrollHeight;
      const clientHeight = document.documentElement.clientHeight;

      // Trigger only when within 20px of the very bottom
  ...
  /* ── Dismiss footer on scroll-up gesture ── */
